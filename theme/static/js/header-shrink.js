(function () {
  ...
  function animateToShrunk() {
    ...
    tl.to('.header-inner', { marginTop: '-5px', height: '100%', duration: 0.2, ease: 'power2.out' })
      ...
      .to('.frm', { y: '-99px', duration: 0.5, ease: "power2.out" }, '<')
  ...
  function animateToExpanded() {
    ...
    // najpierw animacja headera i innych elementów
    const tl = gsap.timeline();
    ...
    gsap.to('.frm', {
      ...
      delay: 0.5,
      onUpdate: function () {
        // sprawdzamy pozycję frm w czasie animacji
        const y = gsap.getProperty('.frm', 'y');
        // jeśli frm osiągnęło 0 lub blisko 0, animujemy lang
        if (y >= -0.5) {
          gsap.to('.lang', { y: 0, duration: 0.2, ease: "power2.out" });
        }
      },
      onComplete: function () {
        // upewniamy się, że lang jest na końcu w pozycji 0
        gsap.set('.lang', { y: 0 });
      }
    });
